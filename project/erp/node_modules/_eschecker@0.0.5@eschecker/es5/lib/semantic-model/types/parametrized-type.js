...
exports.ParametrizedType = undefined;
 ...
 */
class ParametrizedType extends _type.Type {
  ...
  * @returns {boolean} true if it is a subtype, false otherwise
	...
	isSubType(t) {
		if (this.constructor === t.constructor) {
			return (0, _lodash.zip)(this.typeParameters, t.typeParameters).every(_ref => {
  ...
  * @abstract
	...
	withTypeParameters(typeParameters, id) {
		...
		_assert2.default.fail("abstract function withTypeParameters");
	}
	...
	substitute(oldType, newType) {
		...
		const typeParameters = _immutable2.default.fromJS(this.typeParameters);
		const substituted = typeParameters.map(param => param.substitute(oldType, newType));

		// type parameters have not changed
		if (_immutable2.default.is(typeParameters, substituted)) {
			return this;
		}
  ...
  * @returns {boolean} true if the parametrized types are equal
	...
	equals(other) {
		...
		return (0, _lodash.zip)(this.typeParameters, other.typeParameters).every(_ref3 => {
...
//# sourceMappingURL=parametrized-type.js.map
