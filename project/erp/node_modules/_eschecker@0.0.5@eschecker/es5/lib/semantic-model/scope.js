...
class Scope {
  ...
  * @param {Symbol} symbol the symbol to add
     */
	addSymbol(symbol) {
		(0, _assert2.default)(symbol, "symbol cannot be undefined");
		(0, _assert2.default)(!this._symbols.has(symbol.name), `Another symbol with the name ${ symbol.name } already exists`);
  ...
  * @param {Symbol} that the symbol that replaces the thiz symbol
	...
	replaceSymbol(thiz, that) {
		(0, _assert2.default)(that, "that cannot be undefined", "!!");
		(0, _assert2.default)(this._symbols.has(thiz.name), "Can only replaces symbol contained in this scope", "symbols.has(thiz.name)");
		(0, _assert2.default)(thiz.name === that.name, "The name of the symbols need to be equal", "===");
  ...
  * @param {string} [intend=''] intention to use for this scope
     */
	dump(stream) {
		let intend = arguments.length <= 1 || arguments[1] === undefined ? "" : arguments[1];
