...
exports.SymbolExtractor = undefined;
...
class SymbolExtractor {
  ...
  */
	enterFunctionDeclaration(path) {
		...
		if (path.node.id) {
			symbol = this._declareIdentifierInScope(path.get("id").node, _symbol.SymbolFlags.Function | _symbol.SymbolFlags.Hoisted);
			symbol.declaration = symbol.valueDeclaration = path.node;
		} else {
			symbol = new _symbol2.default(uniqueNodeName(path.node, "anonymous"), _symbol.SymbolFlags.Function);
			symbol.declaration = symbol.valueDeclaration = path.node;
		}
		...
		for (const param of path.node.params) {
			const paramSymbol = this._declareIdentifierInScope(param, _symbol.SymbolFlags.Variable);
  ...
  */
	enterVariableDeclarator(path) {
		const symbol = this._declareIdentifierInScope(path.node.id, _symbol.SymbolFlags.Variable);
	...
	enterThisExpression(path) {
		...
		if (!thiz) {
			thiz = new _symbol2.default("this", _symbol.SymbolFlags.Variable);
  ...
  * @constructor
		...
		// If the parent is an assignment expression, then the object is the left hand side of the assignment (the target)
			...
			// If the parent is an object property, then this is a nested object
				...
				// it's an anonymous symbol in a function, return statement, call statement...
						...
						object = new _symbol2.default(uniqueNodeName(parentPath.node, "anonymous object"), _symbol.SymbolFlags.Anonymous);
					}
	...
	enterAssignmentExpression(path) {
		...
		if (path.get("left").isIdentifier()) {
			assignee = this._resolveIdentifier(path.node.left, _symbol.SymbolFlags.Variable);
  ...
  * @private
	...
	_declareIdentifierInScope(identifierNode, flags) {
		...
		if (this.scope.hasOwnSymbol(identifierNode.name)) {
			...
			symbol = new _symbol2.default(identifierNode.name, flags);
			this.scope.addSymbol(symbol);

			if ((flags & _symbol.SymbolFlags.Hoisted) === _symbol.SymbolFlags.Hoisted) {
	...
	_resolveUnresolvedSymbols(resolvedSymbol) {
		let scope = arguments.length <= 1 || arguments[1] === undefined ? this.scope : arguments[1];
	...
	_resolveIdentifiers() {
		...
		identifiers.forEach(identifier => this._resolveIdentifier(identifier.node, _symbol.SymbolFlags.Variable));
	}

	_resolveIdentifier(identifierNode) {
		let flags = arguments.length <= 1 || arguments[1] === undefined ? _symbol.SymbolFlags.UNKNOWN : arguments[1];
	...
	_resolveMember(memberPath) {
		...
		if (memberPath.get("object").isIdentifier()) {
			object = this._resolveIdentifier(memberPath.node.object, _symbol.SymbolFlags.Variable);
			...
			// e.g. "test".length, call({});
			object = new _symbol2.default(uniqueNodeName(memberPath.node, "anonymous"), _symbol.SymbolFlags.Anonymous);
		...
		if (property.isLiteral()) {
			member = new _symbol2.default(property.node.value, _symbol.SymbolFlags.Property);
		} else if (memberPath.node.computed) {
			member = _symbol2.default.COMPUTED;
		} else if (property.isIdentifier()) {
			member = new _symbol2.default(property.node.name, _symbol.SymbolFlags.Property);
		}
	...
	_declareObjectMember(path) {
		...
		if (!object.hasMember(name)) {
			symbol = new _symbol2.default(name, _symbol.SymbolFlags.Property);
...
exports.default = SymbolExtractor;
