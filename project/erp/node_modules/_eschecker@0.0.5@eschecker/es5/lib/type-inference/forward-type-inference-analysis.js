...
exports.ForwardTypeInferenceAnalysis = undefined;
...
var _hindleyMilner = require("./hindley-milner");

var _hindleyMilnerContext = require("./hindley-milner-context");

var _hindleyMilnerDataFlowAnalysis = require("./hindley-milner-data-flow-analysis");

var _typeInferenceContext = require("./type-inference-context");

var _builtInVariables = require("../semantic-model/built-in-variables");

var _util = require("../util");

var _functionDeclarationVisitor = require("./function-declaration-visitor");
...
class ForwardTypeInferenceAnalysis {
  ...
  * @param {HindleyMilner} hindleyMilner the hindley milner instance to use
     */
	constructor(program) {
		let hindleyMilner = arguments.length <= 1 || arguments[1] === undefined ? new _hindleyMilner.HindleyMilner() : arguments[1];
	...
	getDefaultTypeEnvironment() {
		const typeEnvironment = _typeEnvironment2.default.EMPTY;
		return (0, _builtInVariables.addsTypesOfBuiltInVariables)(this._program.globalScope, typeEnvironment);
	}
  ...
  * @returns {Map<ASTNode, TypeEnvironment>} the type environments for the analysed nodes
	...
	analyseSourceFile(sourceFile) {
		let typeEnvironment = arguments.length <= 1 || arguments[1] === undefined ? this.getDefaultTypeEnvironment() : arguments[1];

		const cfg = sourceFile.ast.cfg;

		const functionDeclarationVisitor = new _functionDeclarationVisitor.FunctionDeclarationVisitor(this._program, typeEnvironment);
		(0, _babelTraverse2.default)(sourceFile.ast.program, (0, _util.createTraverseVisitorWrapper)(functionDeclarationVisitor));
		typeEnvironment = functionDeclarationVisitor.typeEnvironment;

		const workListAnalyser = new _hindleyMilnerDataFlowAnalysis.HindleyMilnerDataFlowAnalysis(this, typeEnvironment);
  ...
  * @returns {TypeInferenceContext} the created context
	...
	createTypeInferenceContext(typeEnvironment) {
		return new _typeInferenceContext.TypeInferenceContext(this._program, typeEnvironment);
	}
  ...
  * @returns {HindleyMilnerContext} the created hindley milner context
	...
	createHindleyMilnerContext(inferenceContextOrTypeEnvironment) {
		...
		const inferenceContext = inferenceContextOrTypeEnvironment instanceof _typeInferenceContext.TypeInferenceContext ? inferenceContextOrTypeEnvironment : this.createTypeInferenceContext(inferenceContextOrTypeEnvironment);

		return new _hindleyMilnerContext.HindleyMilnerContext({
			infer(node, context) {
...
exports.ForwardTypeInferenceAnalysis = ForwardTypeInferenceAnalysis;
exports.default = ForwardTypeInferenceAnalysis;
//# sourceMappingURL=forward-type-inference-analysis.js.map
