...
exports.CallExpressionRefinementRule = undefined;
 ...
 */
class CallExpressionRefinementRule {
  ...
  * @param {HindleyMilnerContext} callerContext
	...
	refine(node, callerContext) {
		...
		if (functionType instanceof _types.AnyType) {
  ...
  * of the function is complete. Therefor a simple check if the function can be invoked with the given arguments is sufficient
	...
	_handleExternalDeclaredFunction(functionType, callExpression, callerContext) {
		...
		if (!functionType.thisType.isSubType(thiz)) {
			throw new _typeInferenceError.TypeInferenceError(`The function cannot be called with this of type '${ thiz }' whereas '${ functionType.thisType }' is required.`, callExpression);
		}
		...
		const functionSymbol = new _symbol.Symbol("External declared function");
		...
		for (let i = 0; i < functionType.params.length; ++i) {
			...
			let argumentType = argument ? callerContext.infer(argument) : _types.VoidType.create();
			...
			if (parameterType instanceof _types.FunctionType && argumentType instanceof _types.FunctionType && argumentType.declaration && !parameterType.declaration) {
				...
				if (!updatedExpectedReturnType.isSubType(actualReturnType)) {
					throw new _typeInferenceError.TypeInferenceError(`The return type '${ actualReturnType }' of the callback is not a subtype of the return type '${ updatedExpectedReturnType }' of the expected callback.`, argument);
				}
			} else {
				if (argument) {
				...
				if (!parameterType.isSubType(argumentType)) {
					throw new _typeInferenceError.TypeInferenceError(`The argument ${ i + 1 } with type '${ argumentType }' is not a subtype of the required parameter type '${ parameterType }'.`, callExpression.arguments[i] || callExpression);
				}
			}
		}
  ...
  * Handles a function where the body is known
	...
	_handleFunctionWithDeclaration(node, callerContext, functionType) {
		...
		if (equalInvocation) {
			return equalInvocation.context.getType(_symbol.Symbol.RETURN);
		}
	...
	_invokeFunction(functionType, invocation, callerContext) {
		...
		callContext.setType(_symbol.Symbol.RETURN, functionType.returnType.fresh());
		...
		return callContext.getType(_symbol.Symbol.RETURN);
	}
	...
	_analyzeFunctionBody(functionDeclaration, callContext) {
		...
		if (functionDeclaration.expression) {
			...
			callContext.setType(_symbol.Symbol.RETURN, returnType);
		} else {
	...
	_getCalleeType(calleeContext, node) {
		...
		if (!(functionType instanceof _types.FunctionType || functionType instanceof _types.AnyType)) {
			throw new _typeInferenceError.TypeInferenceError(`Cannot invoke the non function type ${ functionType }.`, node);
		}
  ...
  * @returns {Symbol|undefined} the symbol or undefined
	...
	_endRecursion(functionType) {
		if (!(functionType.returnType instanceof _types.TypeVariable)) {
  ...
  * @param invocation the invocation
	...
	_declareParameters(invocation) {
		...
		for (let i = 0; i < n; ++i) {
			const argumentType = i < invocation.argumentTypes.length ? invocation.argumentTypes[i] : _types.VoidType.create();
	...
	_updateTypes(invocation, callerContext) {
		for (let i = 0; i < invocation.argumentTypes.length; ++i) {
			...
			if (!(argumentType instanceof _types.ObjectType)) {
		...
		const symbolsToIgnore = [_symbol.Symbol.RETURN, ...(thisSymbol ? [] : [thisSymbol])];
...
exports.CallExpressionRefinementRule = CallExpressionRefinementRule; /**
...
class Invocation {
	...
	equals(other) {
		...
		return _.zip(this.argumentTypes, other.argumentTypes).every(_ref => {
...
exports.default = CallExpressionRefinementRule;
//# sourceMappingURL=call-expression-refinement-rule.js.map
