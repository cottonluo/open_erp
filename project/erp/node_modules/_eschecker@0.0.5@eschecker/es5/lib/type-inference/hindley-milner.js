...
exports.HindleyMilner = undefined;
...
var _typeUnificator2 = _interopRequireDefault(_typeUnificator);
...
class HindleyMilner {
  ...
  * @param {RefinementRule[]} [refinementRules] the rules that should be applied to refine the types
     */
	constructor() {
		let unificator = arguments.length <= 0 || arguments[0] === undefined ? new _typeUnificator2.default() : arguments[0];
		let refinementRules = arguments.length <= 1 || arguments[1] === undefined ? (0, _util.globRequireInstances)("./refinement-rules/*-refinement-rule.js", module) : arguments[1];
   ...
   * @type {RefinementRule[]}
   */
		this.refinementRules = _immutable2.default.Set.of(...refinementRules);
	}
  ...
  * @returns {Type} the type of the node.
     */
	infer(e, context) {
		(0, _assert2.default)(e, "A node for which the types should be inferred is required");
		(0, _assert2.default)(context, "A type inference context is required");
  ...
  * if the types are relative to each other (t1=T, t2=S->T, in this case t1 cannot be expressed by T2, neither can t2 be expressed by t1)
	...
	unify(t1, t2, node, context) {
		(0, _assert2.default)(t1, "The type t1 to unify needs to be specified");
		(0, _assert2.default)(t2, "The type t2 to unify needs to be specified");
		(0, _assert2.default)(node, "The node for which the types are to be unified needs to be specified");
		(0, _assert2.default)(context, "The context for the unification is not optional");
		...
		try {
			...
			throw new _typeInferenceError.TypeInferenceError(e, node);
		}
	}
  ...
  * @param {HindleyMilnerContext} context the type inference context that provides the type environment for the merge
	...
	mergeWithTypeEnvironments(others, node, context) {
		for (const other of others) {
			...
			for (const _ref of other.mappings) {
	...
	_getRefinementRule(node) {
		...
		switch (possibleRules.length) {
			case 0:
				throw new _typeInferenceError.TypeInferenceError(`There exists no refinement rule that can handle a node of type ${ node.type }.`, node);
			case 1:
				return possibleRules[0];
			default:
				var ruleNames = possibleRules.map(rule => rule.constructor.name).join();
				throw new _typeInferenceError.TypeInferenceError(`The refinement rule to use for a node of type ${ node.type } is ambiguous (${ ruleNames }).`, node);
		}
	}
}

exports.HindleyMilner = HindleyMilner;
exports.default = HindleyMilner;
//# sourceMappingURL=hindley-milner.js.map
