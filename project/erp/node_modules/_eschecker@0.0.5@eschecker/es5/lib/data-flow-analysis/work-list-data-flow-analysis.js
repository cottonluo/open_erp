...
exports.WorkListDataFlowAnalysis = undefined;
 ...
 */
class WorkListDataFlowAnalysis {
  ...
  * @param {*} [node] optional node value. If specified, then the analysis is only performed from this node to all it's exit nodes
	...
	analyse(cfg, node) {
		...
		while (workList.size) {
			var _workList = (0, _slicedToArray3.default)(workList, 1);

			const node = _workList[0];
  ...
  */
	createEmptyLattice() {
		_assert2.default.fail("Abstract, implementation in subclass required");
	}
  ...
  * @param second the second out state
	...
	_joinInStatesFromPredecessors(node, outSets) {
		...
		const head = outValues[0];
		const tail = outValues.slice(1);
...
exports.WorkListDataFlowAnalysis = WorkListDataFlowAnalysis;
exports.default = WorkListDataFlowAnalysis;
//# sourceMappingURL=work-list-data-flow-analysis.js.map
